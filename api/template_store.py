...
<div class="listing-media" style="--listing-media-height: {{ height }}px" data-default-tab="{{ 'photos' if image_paths else 'videos' }}">
  ...
  <div class="listing-media-panel listing-media-panel--photos{% if not image_paths %} d-none{% endif %}">
    ...
    <img src="{{ media_url(path) }}" alt="" class="listing-media-slide{% if loop.first %} is-active{% endif %}" loading="lazy" onerror="{{ img_fallback }}">
    {% endfor %}
  ...
  <div class="listing-media-panel listing-media-panel--videos{% if not video_paths %} d-none{% endif %}">
    ...
    <div class="listing-media-video{% if loop.first %} is-active{% endif %}">
      <video controls playsinline preload="metadata" src="{{ media_url(path) }}"></video>
    </div>
    {% endfor %}
  </div>
</div>
...
{% block extra_css %}
<style>
  /* About page: shared composition width, clearer rhythm, equal founder cards. */
  .about-page-shell {
    max-width: 1100px;
    margin-inline: auto;
    width: 100%;
  ...
  #leadership .leadership-card {
    ...
    overflow: hidden;
    position: relative;
    padding: clamp(1.5rem, 2.5vw, 2.1rem) clamp(1rem, 2vw, 1.5rem);
    opacity: 1 !important;
  ...
  #leadership .founder-photo-wrap {
    ...
    box-shadow: 0 12px 28px rgba(20, 25, 36, 0.14), 0 0 0 2px rgba(235, 123, 47, 0.28);
  }
  /* Keep portraits aligned and cropped consistently inside equal cards. */
  ...
  #leadership .leadership-copy > .text-muted {
    line-height: 1.65;
    font-size: 0.95rem;
  }
  .about-values {
  ...
  .about-values .about-info-card {
    min-height: 100%;
  }
  .about-values .about-info-card h3 {
    font-family: "Montserrat", sans-serif;
    font-weight: 700;
    line-height: 1.25;
  }
  .about-values .about-info-card p {
    line-height: 1.65;
  }
  .about-stats-section {
    padding-block: clamp(2rem, 4vw, 3.25rem);
...
{% endblock %}
{% block content %}
<section class="about-hero jk-flow">
  <div class="container">
...
<section class="container jk-flow about-page-shell" id="leadership">
  <h2 class="section-title section-title-center">Leadership</h2>
...
<section class="container jk-flow about-page-shell about-values">
...
<section class="about-stats-section">
  <div class="container about-page-shell">
...
{% if media.images and media.images|length %}
  {% set cover = media_url(media.images[0].file_path, external=True) %}
{% elif property.primary_image %}
  {% set cover = media_url(property.primary_image, external=True) %}
{% else %}
  {% set cover = url_for('static', filename='img/default-property.jpg', _external=True) %}
{% endif %}
...
<div class="container py-5 detail-page jk-flow" data-property-id="{{ property.id }}" data-property-area="{{ property.area_name }}, Surat">
  <div class="row g-4">
    <div class="col-lg-7 reveal-on-scroll">
      ...
      {% set img_fallback = "this.onerror=null;this.src='/static/img/default-property.jpg';" %}
      {% if imgs %}
      <img id="mainImg" class="gallery-main mb-2" src="{{ media_url(imgs[0].file_path) }}" alt="" onerror="{{ img_fallback }}">
      <div class="d-flex gap-2 flex-wrap">
        {% for img in imgs %}
        <img class="gallery-thumb {% if loop.first %}active{% endif %}" src="{{ media_url(img.file_path) }}" data-full="{{ media_url(img.file_path) }}" onerror="{{ img_fallback }}">
        {% endfor %}
      </div>
      {% elif property.primary_image %}
      <img class="gallery-main" src="{{ media_url(property.primary_image) }}" alt="" onerror="{{ img_fallback }}">
      {% else %}
      ...
      <video class="w-100 rounded mb-2" controls src="{{ media_url(v.file_path) }}"></video>
      {% endfor %}
      {% endif %}
    </div>
    <div class="col-lg-5 reveal-on-scroll">
      ...
      <a href="{{ media_url(d.file_path) }}" target="_blank" class="d-block"><i class="bi bi-file-pdf"></i> {{ d.doc_name }}</a>
      {% endfor %}
      {% endif %}
    </div>
  </div>
...
<div class="container py-5 jk-flow">
  ...
  {% if submissions %}
  <div class="table-responsive content-card my-listings-table-wrap">
    <table class="table align-middle mb-0">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            {% set st = (s.display_status or s.status or 'pending')|lower %}
            <span class="badge {% if st == 'approved' %}text-bg-success{% elif st == 'rejected' %}text-bg-danger{% elif st in ['sold', 'rented'] %}text-bg-secondary{% else %}text-bg-warning{% endif %}">
              {{ st|title }}
            </span>
          </td>
